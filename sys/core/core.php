...
class Core {
	...
	**/
	public static function libraries($class = false){
		...
		if (isset(self::$_LIB[$class])) return;
		//  check if file exists and load it.
		if (!file_exists(LIBS.$class.EXT)) self::error('LIB404','LIBTIT',array('',$c));
		require LIBS.$class.EXT;
		//  Add the library to the loaded libs array.
		self::$_LIB[$class] = true;
	...
	**/
	...
	public static function debug($backtrace=false, $return=false){
		...
		for ($i=count($backtrace)-1; $i>=1; --$i){
			...
			$fl = isset($backtrace[$i]['file'])? str_replace(PATH,'',$backtrace[$i]['file']) : '';
			$c = (count($e=explode('/',$fl))>=2)? 2:1; 
			$info.= $e[count($e)-$c].($fl?'/':'').str_ireplace(EXT,'',end($e));
	...
	private static function _debug_args($args,$r='',$recur=false){
		...
		for ($i=0; $i<($c=count($args)); $i++){
			if (!is_array($args[$i])){
				if (is_bool($args[$i])) $r.=$args[$i]?'true':'false';
				else $r.="'".$args[$i]."'";
			}   else $r=self::_debug_args($args[$i],$r,true);
			if ($i<($c-1)) $r.=',';
		}
		if ($recur) $r.=')';
		return $r;
	...
	**/
	public static function error($message='DEFMSG', $title='DEFTIT', $var=false, $dg=false, $template='error'){
		...
		if ($fl) $fl = str_ireplace(EXT,'',substr($fl,strrpos($fl,'/')+1));
		$line = "$fl:$ln";
	...
	**/
	public static function errorphp($severity, $message, $path, $line) {
		...
		//  We hide the full server path for security reasons.
		if (strpos($path,'/')!==false) {
			$x    = explode('/', $path);
			$path = $x[count($x)-2].'/'.end($x);
		}
