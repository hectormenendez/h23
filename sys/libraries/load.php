...
class Load {
	...
	**/
	private static function _loadfile($name, $vars, $__return, $ispath=false){
		...
		self::$_VAR = array_merge(self::$_VAR, $vars);
		extract(self::$_VAR);
	...
	public static function model($model){
		// if $model is an array run this method for each value
		if (is_array($model)){
			foreach ($model as $m) self::model($m);
			return
		}
		if ($model='') return;
		// if the model is in a folder parse out the filename.
		if (strpos($model, '/')===false) $path ='';
		else {
